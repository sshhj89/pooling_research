...
parser.add_argument('--ptype', dest='ptype', default="skip", # "skip": original resnet18. "gauss_CN", "max"
                    help='pooling type')
parser.add_argument('--dataset', dest='dataset', type=str, default="Imagenet", #Imagenet(Imagenet-S), Coco, Pascal, Imagenet_car
                    help='dataset type')
...
def main():
    ''' ptype: max and use_mask: True means training with foreground focus
        ...
        python3 main_training_imagenet.py --dataset Imagenet --ptype skip --epochs 120 --batch-size 320 --device 0 --lr 0.125
    ...
    if args.dataset == "CIFAR10":
        ...
        testloader_imagenet = DataLoader(test_data_imagenet, batch_size=args.batch_size,
                                         shuffle=False, num_workers=args.workers)
        test_loaders["Imagenet"] = testloader_imagenet
    ...
    if "resnet" in args.arch:
        if "max" in args.ptype or "skip" in args.ptype:
            try:
                ...
                model = eval(model_path).resnet18(pretrained=False, progress=True, num_classes=num_classes, ptype=args.ptype)
            except ImportError:
    ...
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=30, gamma=0.1) #torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs*1.1)
    ...
    for epoch in range(0, args.epochs):  # loop over the dataset multiple times
        ...
        for batch_idx, data in enumerate(trainloader):
            ...
            if args.ptype == "lift":
                ...
                cu = mse_loss(s,xo)
                cp = mse_loss(d,xe)
                loss = criterion(outputs, labels) + lambda_u *cu  + lambda_p * cp
            ...
            with torch.no_grad():
                ...
                for data_name, test_data_loader in test_loaders.items():
                    ...
                    if data_name == "cifar10":
                        ...
                        if best_prec_imagenet < (mean_accuracy / num_classes):
                            best_prec_imagenet = max(mean_accuracy / num_classes, best_prec_imagenet)

                            save_checkpoint({
                                'epoch': epoch + 1,
                                'state_dict': model.state_dict(),
                                'best_prec': best_prec_imagenet,
                            }, filename=os.path.join(args.save_dir, 'Imagenet_checkpoint_{}.tar'.format(epoch)))
                        logger.info(f'Accuracy of the network on the {data_name} images: {best_prec_imagenet:.2f} %')
    ...
    if args.dataset == "CIFAR10":
        ...
        logger.info(f"Final best_prec_imagenet: {best_prec_imagenet:.2f} %")
